...
| :--- | :---: | :---: | :---: |
| **Độ chính xác Lọc câu hỏi (Triage Accuracy)** | **100.0%** (20/20) | $\ge 85.0\%$ | ✅ ĐẠT |
| **Khớp hoàn hảo cả 3 tiêu chí (Full Match)** | **50.0%** (10/20) | $\ge 75.0\%$ | ⚠️ CHẤP NHẬN ĐƯỢC |
...
| **Độ chính xác Gom cụm Chủ đề (Topic)** | **85.0%** | $\ge 80.0\%$ | ✅ ĐẠT |
| **Độ chính xác Mức khẩn cấp (Urgency)** | **55.0%** | $\ge 70.0\%$ | ⚠️ |
| **Thời gian phản hồi trung bình (Latency)** | **1.82s** / tin nhắn | <= 3.0s | ✅ ĐẠT |
...
| :--- | :---: | :---: |
| **Thực tế: Là câu hỏi cần TA** | **TP = 15** | **FN = 0** |
| **Thực tế: Không cần TA** | **FP = 0** | **TN = 5** |
...
| **TC17** | E403_04 | Lớp 2: Mơ hồ / Thiếu thông tin | `Q=True / Ngoài phạm vi / Không liên quan / none` | `Q=True / Kỹ thuật & Bài Lab / urgent` | ⚠️ Lệch phụ |
...
- **Lý do AI đưa ra:** Học viên hỏi về việc xem lại record các buổi workshop, không liên quan đến các vấn đề kỹ thuật hay logistics cụ thể.
- **Mổ xẻ nguyên nhân kỹ thuật:**
...
- **Lý do AI đưa ra:** Học viên yêu cầu kiểm tra điểm danh và hỗ trợ điểm danh bù cho buổi học tối hôm nay.
- **Mổ xẻ nguyên nhân kỹ thuật:**
  - *Sai lệch mức khẩn cấp:* Kỳ vọng `urgent` vs AI phán đoán `medium`. Độ khẩn cấp mang tính chủ quan cao giữa các annotator con người và quy tắc heuristic của prompt.
...
- **Lý do AI đưa ra:** Học viên đang hỏi về lỗi provider_error và cách chạy từng test case để khắc phục vấn đề liên quan đến API.
- **Mổ xẻ nguyên nhân kỹ thuật:**
  - *Sai lệch mức khẩn cấp:* Kỳ vọng `urgent` vs AI phán đoán `medium`. Độ khẩn cấp mang tính chủ quan cao giữa các annotator con người và quy tắc heuristic của prompt.
...
- **Lý do AI đưa ra:** Học viên hỏi về thông tin liên quan đến biên tập viên và giảng viên duyệt kịch bản cho khảo sát nhóm.
- **Mổ xẻ nguyên nhân kỹ thuật:**
  - *Sai lệch mức khẩn cấp:* Kỳ vọng `medium` vs AI phán đoán `low`. Độ khẩn cấp mang tính chủ quan cao giữa các annotator con người và quy tắc heuristic của prompt.
...
- **Kỳ vọng (Human Ground Truth):** `is_unanswered_question=True`, `topic_cluster='Kỹ thuật & Bài Lab'`, `urgency_level='urgent'`
- **Thực tế AI trả lời:** `is_unanswered_question=True`, `topic_cluster='Ngoài phạm vi / Không liên quan'`, `urgency_level='none'`
- **Lý do AI đưa ra:** Tin nhắn yêu cầu sự trợ giúp nhưng không liên quan đến nội dung khóa học hoặc vấn đề kỹ thuật.
- **Mổ xẻ nguyên nhân kỹ thuật:**
  - *Sai lệch gom cụm:* Kỳ vọng là `Kỹ thuật & Bài Lab` nhưng mô hình chọn `Ngoài phạm vi / Không liên quan` do sự giao thoa ngữ nghĩa trong câu hỏi.
  - *Sai lệch mức khẩn cấp:* Kỳ vọng `urgent` vs AI phán đoán `none`. Độ khẩn cấp mang tính chủ quan cao giữa các annotator con người và quy tắc heuristic của prompt.
...
3. **Về Tốc độ & Độ tin cậy Kỹ thuật:**
   - Toàn bộ 20 lời gọi API đều hoàn thành với độ trễ trung bình 1.82s, không gặp lỗi HTTP 429 hay timeout. 100% vết gọi API đều được lưu trữ đầy đủ tại thư mục `eval/logs/` dưới định dạng JSON có đầy đủ `prompt_input` và `raw_response`.
