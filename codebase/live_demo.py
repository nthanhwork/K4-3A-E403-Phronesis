...
def run_live_demo():
    ...
    print(
        f"{BOLD}{CYAN}  🤖 PHRONESIS TRIAGE ENGINE - LIVE REALTIME DEMO (CHECKPOINT 3){RESET}"
    )
    print(
        f"{BOLD}{BLUE}========================================================================{RESET}"
    )
    print(
        f"Model: {BOLD}gpt-4o-mini{RESET} | Engine: {BOLD}codebase/triage_engine.py{RESET} | Target: {BOLD}30s Screen Recording{RESET}\n"
    )
    time.sleep(1)
    ...
    for i, case in enumerate(DEMO_CASES, 1):
        ...
        print(
            f"👤 Author: {BOLD}{case['author']}{RESET} | ID: {CYAN}{case['msg_id']}{RESET}"
        )
        print(f"💬 Tin nhắn: \"{case['content']}\"")
        print(
            f"{YELLOW}⏳ Đang gửi request tới OpenAI API trực tiếp...{RESET}",
            end="",
            flush=True,
        )
        ...
        start_t = time.time()
        res = analyze_message(
            case["content"], author=case["author"], msg_id=case["msg_id"]
        )
        ...
        q_badge = (
            f"{GREEN}CẦN TA HỖ TRỢ (True){RESET}"
            if is_q
            else f"{RED}BỎ QUA / KHÔNG CẦN TA (False){RESET}"
        )
        ...
        recent_logs = sorted(
            [f for f in os.listdir(LOG_DIR) if f.startswith(case["msg_id"])]
        )
        if recent_logs:
